...
common_translate_constraints = [
    "Ensure the Rust code is safe and idiomatic, include all necessary imports, and avoid using raw pointers.",
    "Prefer Box pointers over other alternatives when possible.",
    "Use Rust's preferred data types (such as i32 instead of int and int32_t, etc.), ensure that using a stable compiler will not result in errors.",
    "For bitwise operations, consider using \'%\' and \'as\' to handle the range of shifts and types of shift amounts.",
    "Maintain consistency in function names across translations and exclude the main function.",
]
...
def main(options):
    ...
    if options.language == "c":
        global_constraints.append("Use overflow handling methods (such as wrapping *, etc.) as much as possible to simulate the C semantics of integer operations.")
    ...
    if best_candidate.positive_examples is not None and best_candidate.positive_examples is not None:
        if not best_candidate.positive_examples: # positive_examples == []
            signals = "-(1-3)-\n"  # all negative_examples
        else:
            signals = "-(1-4)-\n"  # positive_examples and negative_examples
        with open(f"{res_all}/fail.txt", 'a') as file:
            file.write(res_path+signals)
    else:
        signals = "-(1-5)-\n"  # no positive_examples and no negative_examples
        with open(f"{res_all}/fail.txt", 'a') as file:
            file.write(res_path+signals)
...
if __name__ == "__main__":
    ...
    # Record the running parameters to setting.txt
    with open(f"{setting_option.work_dir}/setting.txt", 'w', encoding='utf-8') as f:
        for field, value in asdict(setting_option).items():
