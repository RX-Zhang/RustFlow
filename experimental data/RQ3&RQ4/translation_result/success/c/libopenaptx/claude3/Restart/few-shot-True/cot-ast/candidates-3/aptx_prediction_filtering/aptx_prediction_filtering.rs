...
const fn diff_sign(x: i32, y: i32) -> i32 {
    match x.cmp(&y) {
        Ordering::Greater => 1,
        Ordering::Less => -1,
        Ordering::Equal => 0,
...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if ((a as u32).wrapping_add(1 << p)) & !((2 << p).wrapping_sub(1)) != 0 {
        (a >> 31) ^ ((1 << p).wrapping_sub(1))
    } else {
        a
    }
}
...
fn rshift32(value: i32, shift: u32) -> i32 {
    ...
    let mask = (1 << (shift.wrapping_add(1))).wrapping_sub(1);
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) {
    ...
    let srd0 = diff_sign(reconstructed_difference, 0).wrapping_mul(1 << 23);
    ...
    for i in 0..order {
        ...
        predicted_difference = predicted_difference.wrapping_add(
            (reconstructed_differences as i64).wrapping_mul(prediction.d_weight[i] as i64)
        );
    }
