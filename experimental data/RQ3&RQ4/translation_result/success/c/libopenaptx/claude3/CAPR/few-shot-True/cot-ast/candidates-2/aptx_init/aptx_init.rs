...
fn aptx_reset(ctx: &mut AptxContext) {
    let hd = ctx.hd;
    unsafe {
        std::ptr::write_bytes(ctx as *mut AptxContext as *mut u8, 0, std::mem::size_of::<AptxContext>());
    }
    ctx.hd = hd;
...
fn aptx_init(hd: bool) -> Option<Box<AptxContext>> {
    let mut ctx = Box::new(unsafe { MaybeUninit::<AptxContext>::uninit().assume_init() });
