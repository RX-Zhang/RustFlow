...
fn rshift32(value: i32, shift: u32) -> i32 {
    ...
    let mask = ((1i32 << (shift + 1)) as u32).wrapping_sub(1) as i32;
...
fn aptx_quantize_difference(quantize: &mut AptxQuantize, sample_difference: i32, dither: i32, quantization_factor: i32, tables: &AptxTables) {
    ...
    let d = rshift32_clip24((dither as i64).wrapping_mul(dither as i64) as i32, 7).wrapping_sub(1 << 23);
    ...
    let mut quantized_sample = if error < 0 { quantized_sample - 1 } else { quantized_sample };
    parity_change -= 1;
