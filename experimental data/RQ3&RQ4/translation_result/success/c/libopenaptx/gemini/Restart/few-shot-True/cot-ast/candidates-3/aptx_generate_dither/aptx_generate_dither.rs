...
fn aptx_update_codeword_history(channel: &mut AptxChannel) {
    ...
    channel.codeword_history =
        (cw << 8) + ((channel.codeword_history as u32).wrapping_shl(4) as i32);
}
...
fn aptx_generate_dither(channel: &mut AptxChannel) {
    ...
    let d = ((m.wrapping_mul(4)) + (m.wrapping_shr(22))) as i32;
    for subband in 0..NB_SUBBANDS {
        channel.dither[subband] =
            (d as u32).wrapping_shl((23 - 5 * subband) as u32) as i32;
    }
