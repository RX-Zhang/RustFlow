...
fn opl_emu_attenuation_increment(rate: u32, index: u32) -> u32 {
    ...
    let safe_rate = Wrapping(rate);
    let safe_index = Wrapping(index);
    let table_index = (safe_rate % Wrapping(64)).0 as usize;
    let bit_start = (safe_index % Wrapping(8) * Wrapping(4)).0;
    
    opl_emu_bitfield(INCREMENT_TABLE[table_index], bit_start as i32, 4)
}
