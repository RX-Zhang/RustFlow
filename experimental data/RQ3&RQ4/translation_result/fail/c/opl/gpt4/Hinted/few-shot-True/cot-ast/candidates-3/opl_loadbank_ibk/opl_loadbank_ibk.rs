...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> i32 {
    let mut instruments = CString::new(file).unwrap().into_bytes_with_nul();
    ...
    let res = opl_loadbank_internal(opl, std::str::from_utf8(&instruments).unwrap(), 0);
    if res == 0 {
        if let Some(percussion) = percussion {
            let percussion_str = std::str::from_utf8(percussion).unwrap();
            return opl_loadbank_internal(opl, percussion_str, 128);
        }
    }
    res
}
