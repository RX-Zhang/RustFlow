...
fn opl_emu_bitfield(value: u32, start: u32, length: u32) -> u32 {
    let mask = (1u32.wrapping_shl(length)) - 1;
...
fn opl_emu_registers_byte(regs: &OplEmuRegisters, offset: usize, start: u32, count: u32, extra_offset: usize) -> u32 {
    opl_emu_bitfield(regs.m_regdata[offset.wrapping_add(extra_offset) % OPL_EMU_REGISTERS_REGISTERS] as u32, start, count)
}

fn opl_emu_registers_op_attack_rate(regs: &OplEmuRegisters, opoffs: usize) -> u32 {
