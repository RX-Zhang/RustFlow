...
fn opl_loadbank_internal(opl: &mut Opl, file: &str, offset: usize) -> i32 {
    ...
    if f.read(&mut buff[0..4]).unwrap() != 4 || buff[0] != b'I' || buff[1] != b'B' || buff[2] != b'K' || buff[3] != 0x1A {
        return -3;
    }
    ...
    for i in offset..128 + offset {
        ...
        opl.opl_gmtimbres[i].modulator_e862 = ((buff[8] as u32) << 24) | ((buff[6] as u32) << 16) | ((buff[4] as u32) << 8) | (buff[0] as u32);
        opl.opl_gmtimbres[i].carrier_e862 = ((buff[9] as u32) << 24) | ((buff[7] as u32) << 16) | ((buff[5] as u32) << 8) | (buff[1] as u32);
...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> i32 {
    let mut instruments = match CString::new(file) {
        Ok(cstr) => cstr.into_bytes_with_nul(),
        Err(_) => return -64,
    };
    ...
    for i in 0..instruments.len() {
        if instruments[i] == b',' {
            instruments[i] = 0;
            percussion = Some(&file[i + 1..]);
            break;
        }
    }

    let res = opl_loadbank_internal(opl, &String::from_utf8_lossy(&instruments), 0);
    if res == 0 {
        if let Some(percussion_file) = percussion {
            return opl_loadbank_internal(opl, percussion_file, 128);
        }
    }
    res
}
