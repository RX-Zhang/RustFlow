...
fn opl_emu_fm_operator_clock_envelope(fmop: &mut OplEmuFmOperator, env_counter: u32) {
    ...
    let mut env_counter = env_counter << rate_shift;
    if opl_emu_bitfield(env_counter, 0, 11) != 0 {
        return;
    }
    let relevant_bits = opl_emu_bitfield(env_counter, if rate_shift <= 11 { 11 } else { rate_shift.into() }, 3);
    let increment = opl_emu_attenuation_increment(rate as u32, relevant_bits);
    if fmop.m_env_state == OplEmuEnvelopeState::Attack {
        if rate < 62 {
            fmop.m_env_attenuation = fmop.m_env_attenuation.wrapping_add(((!fmop.m_env_attenuation as u32 * increment) >> 4).try_into().unwrap());
        }
    } else {
