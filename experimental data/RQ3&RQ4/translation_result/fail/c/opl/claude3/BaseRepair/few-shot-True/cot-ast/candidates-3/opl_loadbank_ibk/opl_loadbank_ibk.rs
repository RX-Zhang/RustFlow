...
struct OplEmuRegisters {
    ...
    m_noise_lfsr: u32,
    m_lfo_am: u8,
    m_registers: [u8; OPL_EMU_REGISTERS_REGISTERS],
...
fn opl_loadbank_internal(opl: &mut Opl, file: &str, offset: i32) -> i32 {
    ...
    if f.read(&mut buff[..4]).unwrap() != 4 || buff[0] != b'I' || buff[1] != b'B' || buff[2] != b'K' || buff[3] != 0x1A {
        return -3;
    }
    ...
    for i in offset..128 + offset {
        ...
        opl.opl_gmtimbres[i as usize].modulator_e862 = u32::from_le_bytes([buff[0], buff[4], buff[6], buff[8]]);
        opl.opl_gmtimbres[i as usize].carrier_e862 = u32::from_le_bytes([buff[1], buff[5], buff[7], buff[9]]);
...
fn opl_loadbank_ibk(opl: &mut Opl, file: &str) -> i32 {
    let instruments_parts: Vec<&str> = file.split(',').collect();
    let instruments = instruments_parts[0];
    let percussion = instruments_parts.get(1);

    let res = opl_loadbank_internal(opl, instruments, 0);
    
    if res == 0 && percussion.is_some() {
        opl_loadbank_internal(opl, percussion.unwrap(), 128)
    } else {
        res
    }
}
