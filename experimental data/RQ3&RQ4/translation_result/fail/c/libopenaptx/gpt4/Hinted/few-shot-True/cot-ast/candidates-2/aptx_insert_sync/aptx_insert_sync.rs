...
fn aptx_insert_sync(channels: &mut [AptxChannel; NB_CHANNELS], sync_idx: &mut u8) {
    ...
    if aptx_check_parity(channels, sync_idx) != 0 {
        for (channel_idx, c) in channels.iter_mut().enumerate() {
            for &i in &map {
        ...
        channels[channel_idx].quantize[quantize_idx].quantized_sample = channels[channel_idx].quantize[quantize_idx].quantized_sample_parity_change;
    }
}
