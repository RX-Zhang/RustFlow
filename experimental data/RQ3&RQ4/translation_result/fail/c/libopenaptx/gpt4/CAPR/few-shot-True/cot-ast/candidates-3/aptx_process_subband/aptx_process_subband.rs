...
fn aptx_reconstructed_differences_update(
    ...
    prediction.reconstructed_differences[p] = prediction.reconstructed_differences[p + order_usize];
    prediction.pos = ((p + 1) % order_usize) as i32;
    prediction.reconstructed_differences[p + order_usize] = reconstructed_difference;
    &mut prediction.reconstructed_differences[p + order_usize]
}
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
    ...
    let rd = aptx_reconstructed_differences_update(prediction, reconstructed_difference, order);
    let srd0 = diffsign(re