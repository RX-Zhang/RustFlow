...
fn aptx_reconstructed_differences_update(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) -> i32 {
    ...
    prediction.reconstructed_differences[prediction.pos as usize + order]
}
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) {
    ...
    let reconstructed_diff = aptx_reconstructed_differences_update(prediction, reconstructed_difference, order);
    let srd0 = diffsign(reconstructed_difference, 0).wrapping_mul(1 << 23);
    let mut predicted_difference: i64 = 0;

    for i in 0..order {
        let idx = prediction.pos as usize;
        let prev_idx = (idx + order - i - 1) % order;
        let srd = (prediction.reconstructed_differences[prev_idx] >> 31) | 1;
        ...
        let current_idx = (idx + order - i) % order;
        predicted_difference = predicted_difference.wrapping_add(
            (prediction.reconstructed_differences[current_idx] as i64)
