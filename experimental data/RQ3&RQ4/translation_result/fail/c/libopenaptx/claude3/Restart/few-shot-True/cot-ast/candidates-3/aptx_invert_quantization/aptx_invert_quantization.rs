...
fn aptx_invert_quantization(
    ...
    let idx = (quantized_sample ^ ((quantized_sample < 0) as i32).wrapping_neg()).wrapping_add(1);
    ...
    qr = rshift64_clip24(
        (qr as i64).wrapping_mul(1i64 << 32).wrapping_add(
            (dither as i64).wrapping_mul(tables.invert_quantize_dither_factors[idx as usize] as i64)
        ),
        32
    );
    
    invert_quantize.reconstructed_difference = ((invert_quantize.quantization_factor as i64)
        .wrapping_mul(qr as i64) >> 19) as i32;
    ...
    factor_select = rshift32(
        factor_select.wrapping_add(
            (tables.quantize_factor_select_offset[idx as usize] as i32).wrapping_mul(1 << 15)
        ),
        15
    );
