...
const OPL_EMU_EG_STATES: usize = OplEmuEnvelopeState::States as usize;
...
fn opl_emu_registers_byte(
    ...
    opl_emu_bitfield(
        regs.m_regdata[(offset + extra_offset) as usize].into(), // 将 u8 转换为 u32
        start as i32,
        count as i32,
    )
}
...
fn opl_emu_update_timer(emu: &mut OplEmuT, tnum: u32, enable: u32) {
    if enable != 0 && emu.m_timer_running[tnum as usize] == 0 {
        let period = if tnum == 0 {
            1024u32.wrapping_sub(opl_emu_registers_timer_a_value(&emu.m_regs))
        } else {
            16u32.wrapping_mul(256u32.wrapping_sub(opl_emu_registers_timer_b_value(&emu.m_regs)))
        };
