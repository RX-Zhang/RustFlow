...
fn aptx_quantize_difference(
    ...
    sample_difference_abs = min(sample_difference_abs, (1 << 23) - 1);
    ...
    let interval_slice = &intervals[quantized_sample as usize..];
    mean = (interval_slice[1] + interval_slice[0]) / 2;
    interval = (interval_slice[1] - interval_slice[0]) * (if sample_difference < 0 { -1 } else { 1 });
