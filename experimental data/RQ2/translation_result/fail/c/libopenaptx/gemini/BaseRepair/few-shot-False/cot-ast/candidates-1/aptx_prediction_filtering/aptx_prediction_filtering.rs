...
fn aptx_prediction_filtering(
    ...
    predictor = clip_intp2(
        (((prediction.s_weight[0] as i64)
            ...
            .wrapping_add(
                (prediction.s_weight[1] as i64).wrapping_mul(reconstructed_sample as i64),
            ))
            >> 22) as i32,
        23,
    );
    ...
    srd0 = (reconstructed_difference.signum() * (1 << 23)) as i32;
    for i in 0..order {
        srd = (reconstructed_differences[-(i + 1) as usize] >> 31) | 1;
        ...
        predicted_difference = predicted_difference.wrapping_add(
            (reconstructed_differences[-i as usize] as i64)
