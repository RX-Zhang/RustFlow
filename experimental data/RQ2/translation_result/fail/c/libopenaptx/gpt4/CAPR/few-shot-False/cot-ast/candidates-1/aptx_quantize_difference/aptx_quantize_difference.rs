...
fn aptx_bin_search(value: i32, factor: i32, intervals: &[i32], nb_intervals: usize) -> i32 {
    ...
    while i > 0 {
        if (factor as i64).wrapping_mul(intervals[idx + i] as i64) <= (value as i64).shl(24) {
            idx += i;
        }
        i >>= 1;
    }
...
fn aptx_quantize_difference(
    ...
    let interval_start = intervals[quantized_sample as usize];
    let interval_end = intervals[quantized_sample as usize + 1];
    mean = (interval_end + interval_start) / 2;
    interval = (interval_end - interval_start) * if sample_difference < 0 { -1 } else { 1 };

    dithered_sample = rshift64_clip24(
        (dither as i64).wrapping_mul(interval as i64) + (clip_intp2(mean + d, 23) as i64).shl(32),
        32,
    );
    error = (sample_difference_abs as i64).shl(20) - (dithered_sample as i64).wrapping_mul(quantization_factor as i64);
