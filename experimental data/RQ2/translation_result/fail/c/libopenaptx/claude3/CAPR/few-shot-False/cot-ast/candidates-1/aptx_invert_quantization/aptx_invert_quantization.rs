...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    let idx = (quantized_sample ^ (if quantized_sample < 0 { -1 } else { 0 })).wrapping_add(1);
    ...
    qr = rshift64_clip24(
        (Wrapping(qr as i64) * Wrapping(1i64 << 32)).0.wrapping_add(
            (Wrapping(dither as i64) * Wrapping(tables.invert_quantize_dither_factors[idx as usize] as i64)).0
        ),
        32
    );
    invert_quantize.reconstructed_difference = ((Wrapping(invert_quantize.quantization_factor as i64) * Wrapping(qr as i64)).0 >> 19) as i32;

    let mut factor_select = (Wrapping(32620) * Wrapping(invert_quantize.factor_select)).0;
    factor_select = rshift32(factor_select.wrapping_add((tables.quantize_factor_select_offset[idx as usize] as i32) * (1 << 15)), 15);
