...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) {
    ...
    for i in 0..order {
        let idx1 = prediction.pos as usize + order - i - 1;
        let idx2 = prediction.pos as usize + order - i;
        let srd = (prediction.reconstructed_differences[idx1] >> 31) | 1;
        ...
        predicted_difference = predicted_difference.wrapping_add(
            (prediction.reconstructed_differences[idx2] as i64).wrapping_mul(prediction.d_weight[i] as i64),
        );
    }
