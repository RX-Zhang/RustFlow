...
fn aptx_quantize_difference(
    ...
    let d = rshift32_clip24((((dither as i64).wrapping_mul(dither as i64)) >> 32) as i32, 7)
        .wrapping_sub(1 << 23);
    let d = rshift64(
    ...
    let mut final_quantized_sample = quantized_sample;
    
    if error < 0 {
        final_quantized_sample -= 1;
    } else {
        parity_change -= 1;
    }

    let inv = if sample_difference < 0 { -1 } else { 0 };
    quantize.quantized_sample = final_quantized_sample ^ inv;
