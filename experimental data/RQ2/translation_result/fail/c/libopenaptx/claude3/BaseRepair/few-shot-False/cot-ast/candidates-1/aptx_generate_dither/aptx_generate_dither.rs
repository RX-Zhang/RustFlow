...
fn aptx_generate_dither(channel: &mut AptxChannel) {
    ...
    let m = Wrapping(5184443_i64).0.wrapping_mul((channel.codeword_history >> 7) as i64);
    let d = m.wrapping_mul(4).wrapping_add(m.wrapping_shr(22) as i64) as i32;
    
    for (subband, dither) in channel.dither.iter_mut().enumerate() {
        *dither = ((d as u32).wrapping_shl(23_u32.wrapping_sub(5_u32.wrapping_mul(subband as u32)))) as i32;
    }
