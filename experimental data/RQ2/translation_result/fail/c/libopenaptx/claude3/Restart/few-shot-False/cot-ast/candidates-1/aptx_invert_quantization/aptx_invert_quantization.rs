...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    ...
    qr = rshift64_clip24(
        ((qr as i64) * (1i64 << 32)).wrapping_add((dither as i64) * (tables.invert_quantize_dither_factors[idx] as i64)),
        32
    );
    ...
    invert_quantize.quantization_factor = (QUANTIZATION_FACTORS[idx] as i32 * (1 << 11)) >> shift;
}
