...
fn opl_emu_bitfield(value: u32, start: u32, length: u32) -> u32 {
    (value.wrapping_shr(start % 32) & ((1u32.wrapping_shl(length % 32)) - 1)) as u32
}
...
fn opl_emu_registers_op_lfo_pm_enable(regs: &OplEmuRegisters, opoffs: usize) -> u32 {
    opl_emu_registers_byte(regs, 0x20, 6, 1, opoffs % OPL_EMU_REGISTERS_REGISTERS)
}
