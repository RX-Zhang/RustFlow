...
pub fn opl_emu_bitfield(value: u32, start: u32, length: u32) -> u32 {
    let mask = (1u32.wrapping_shl(length % 32)).wrapping_sub(1);
    (value.wrapping_shr(start % 32)) & mask
}
...
pub fn opl_emu_registers_op_sustain_level(regs: &OplEmuRegisters, opoffs: usize) -> u32 {
    opl_emu_registers_byte(regs, 0x80, 4, 4, opoffs % OPL_EMU_REGISTERS_REGISTERS)
}
