...
fn aptx_update_codeword_history(channel: &mut Box<AptxChannel>) {
    let cw = ((channel.quantize[0].quantized_sample & 3) << 0).wrapping_add(
        ((channel.quantize[1].quantized_sample & 2) << 1).wrapping_add(
...
fn aptx_generate_dither(channel: &mut Box<AptxChannel>) {
    ...
    let d: i32 = ((m * 4).wrapping_add(m >> 22)) as i32;
    for subband in 0..NB_SUBBANDS {
        channel.dither[subband] = (d as u32).wrapping_shl((23 - (5 * subband as i32)) as u32) as i32;
    }
