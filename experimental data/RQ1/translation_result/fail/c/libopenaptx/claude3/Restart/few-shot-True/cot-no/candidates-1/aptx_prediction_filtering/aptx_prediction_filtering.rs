...
fn aptx_reconstructed_differences_update(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) -> i32 {
    ...
    prediction.reconstructed_differences[p + order]
}
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) {
    ...
    let _reconstructed_differences = aptx_reconstructed_differences_update(prediction, reconstructed_difference, order);
    let srd0 = diffsign(reconstructed_difference, 0) * (1 << 23);
    
    let mut predicted_difference: i64 = 0;
    for i in 0..order {
        let srd = (prediction.reconstructed_differences[((prediction.pos as usize - i - 1 + order) % order) + order] >> 31) | 1;
        prediction.d_weight[i] = prediction.d_weight[i].wrapping_sub(rshift32(prediction.d_weight[i].wrapping_sub(srd * srd0), 8));
        predicted_difference = predicted_difference.wrapping_add(prediction.reconstructed_differences[((prediction.pos as usize - i + order) % order) + order] as i64 * prediction.d_weight[i] as i64);
    }
