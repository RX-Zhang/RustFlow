...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if ((a as u32).wrapping_add(1 << p)) & !((2i32 << p).wrapping_sub(1) as u32) != 0 {
        (a >> 31) ^ ((1 << p) - 1)
    } else {
        a
    }
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
    ...
    for i in 0..order as usize {
        ...
        predicted_difference += prediction.reconstructed_differences[order as usize - i] as i64 * prediction.d_weight[i] as i64;
    }
...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    ...
    qr = rshift64_clip24(((qr as i64) << 32).wrapping_add(dither as i64 * tables.invert_quantize_dither_factors[idx] as i64), 32);
    ...
    factor_select = rshift32(factor_select.wrapping_add(tables.quantize_factor_select_offset[idx] as i32 * (1 << 15)), 15);
