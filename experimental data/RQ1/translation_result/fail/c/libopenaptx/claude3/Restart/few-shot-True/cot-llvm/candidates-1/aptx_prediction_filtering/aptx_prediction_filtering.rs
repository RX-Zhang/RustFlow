...
fn aptx_reconstructed_differences_update(prediction: &mut Box<AptxPrediction>, reconstructed_difference: i32, order: usize) -> [i32; 48] {
    let p = prediction.pos as usize;
    prediction.reconstructed_differences[p] = prediction.reconstructed_differences[order + p];
    prediction.pos = ((p + 1) % order) as i32;
    let new_p = prediction.pos as usize;
    prediction.reconstructed_differences[order + new_p] = reconstructed_difference;
    prediction.reconstructed_differences
}

fn aptx_prediction_filtering(prediction: &mut Box<AptxPrediction>, reconstructed_difference: i32, order: usize) {
    ...
    for i in 0..order {
        let srd = (reconstructed_differences[order - (i + 1)] >> 31) | 1;
        prediction.d_weight[i] = prediction.d_weight[i].wrapping_sub(
            rshift32(prediction.d_weight[i].wrapping_sub(srd.wrapping_mul(srd0)), 8)
        );
        predicted_difference = predicted_difference.wrapping_add(
            reconstructed_differences[order - i] as i64 * prediction.d_weight[i] as i64
        );
    }
