...
const DIFFSIGN: fn(i32, i32) -> i32 = |x, y| match x.cmp(&y) {
    Ordering::Greater => 1,
    Ordering::Less => -1,
    Ordering::Equal => 0,
};
...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if ((a as u32).wrapping_add(1 << p)) & !((2_u32 << p).wrapping_sub(1)) != 0 {
        (a >> 31) ^ ((1 << p) - 1)
    } else {
        a
    }
...
fn aptx_reconstructed_differences_update(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) -> &mut i32 {
    ...
    &mut prediction.reconstructed_differences[prediction.pos as usize + order]
}
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) {
    ...
    for i in 0..order {
        let srd = (prediction.reconstructed_differences[(prediction.pos as usize + order - i - 1) % order] >> 31) | 1;
        ...
        predicted_difference = predicted_difference.wrapping_add(
            prediction.reconstructed_differences[(prediction.pos as usize + order - i - 1) % order] as i64 * prediction.d_weight[i] as i64
        );
    }
