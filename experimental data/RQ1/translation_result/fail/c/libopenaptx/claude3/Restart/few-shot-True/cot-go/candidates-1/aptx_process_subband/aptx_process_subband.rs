...
fn rshift32(value: i32, shift: u32) -> i32 {
    ...
    ((value + rounding) >> shift).wrapping_sub((value & mask == rounding) as i32)
}

fn aptx_reconstructed_differences_update(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) -> Box<i32> {
    ...
    Box::new(prediction.reconstructed_differences[p + order as usize])
}
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: i32) {
    ...
    let predictor = clip_intp2((((prediction.s_weight[0] as i64).wrapping_mul(prediction.previous_reconstructed_sample as i64)
        .wrapping_add((prediction.s_weight[1] as i64).wrapping_mul(reconstructed_sample as i64))) >> 22) as i32, 23);
    ...
    for i in 0..order as usize {
        let srd = (prediction.reconstructed_differences[order as usize - i - 1] >> 31) | 1;
        ...
        predicted_difference = predicted_difference.wrapping_add((prediction.reconstructed_differences[order as usize - i - 1] as i64).wrapping_mul(prediction.d_weight[i] as i64));
    }
...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    ...
    qr = rshift64_clip24(((qr as i64).wrapping_mul(1 << 32)).wrapping_add((dither as i64).wrapping_mul(tables.invert_quantize_dither_factors[idx] as i64)), 32);
...
fn aptx_process_subband(invert_quantize: &mut AptxInvertQuantize, prediction: &mut AptxPrediction, quantized_sample: i32, dither: i32, tables: &AptxTables) {
    ...
    let same_sign = [sign.wrapping_mul(prediction.prev_sign[0]), sign.wrapping_mul(prediction.prev_sign[1])];
    prediction.prev_sign[0] = prediction.prev_sign[1];
    prediction.prev_sign[1] = sign | 1;

    let mut range = 0x100000;
    let mut sw1 = rshift32(-same_sign[1].wrapping_mul(prediction.s_weight[1]), 1);
    sw1 = (clip(sw1, -range, range) & !0xF).wrapping_mul(16);
