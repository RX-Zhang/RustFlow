...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if (a.wrapping_add(1 << p) as u32) & !((2u32 << p) - 1) != 0 {
        (a >> 31) ^ ((1 << p) - 1)
    } else {
        a
    }
...
fn aptx_prediction_filtering(prediction: &mut AptxPrediction, reconstructed_difference: i32, order: usize) {
    ...
    let predictor = clip_intp2(((Wrapping(prediction.s_weight[0] as i64) * Wrapping(prediction.previous_reconstructed_sample as i64)
        + Wrapping(prediction.s_weight[1] as i64) * Wrapping(reconstructed_sample as i64)).0 >> 22) as i32, 23);
    ...
    for i in 0..order {
        ...
        predicted_difference = predicted_difference.wrapping_add(
            (prediction.reconstructed_differences[prediction.pos as usize + order - i] as i64)
