...
impl Default for AptxQmfAnalysis {
    fn default() -> Self {
        Self {
            outer_filter_signal: [Default::default(); NB_FILTERS],
            inner_filter_signal: [[Default::default(); NB_FILTERS]; NB_FILTERS],
        }
    }
}
...
impl Default for AptxChannel {
    fn default() -> Self {
        Self {
            ...
            qmf: Default::default(),
            quantize: [Default::default(); NB_SUBBANDS],
            invert_quantize: [Default::default(); NB_SUBBANDS],
            prediction: [Default::default(); NB_SUBBANDS],
        }
    }
}
...
fn aptx_reset(ctx: &mut AptxContext) {
    let hd = ctx.hd;
    unsafe {
        std::ptr::write_bytes(ctx as *mut AptxContext as *mut u8, 0, size_of::<AptxContext>());
    }
    ctx.hd = hd;
