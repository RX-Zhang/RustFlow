...
fn aptx_quantize_difference(quantize: &mut AptxQuantize,
    ...
    let mut inv: i32;
    let sample_difference_abs = sample_difference.abs();
    let sample_difference_abs = sample_difference_abs.min( (1 << 23) -1 );
    ...
    let intervals_ptr = unsafe { intervals.get_unchecked(quantized_sample as usize) };
    mean = (intervals_ptr[1] + intervals_ptr[0]) / 2;
    interval = (intervals_ptr[1] - intervals_ptr[0]) * (-(sample_difference < 0) | 1);
