    ...
    if aptx_check_parity(channels, sync_idx) != 0 {
        ...
        for c_idx in 0..2 {
            for i in 0..NB_SUBBANDS {
                let mapped_index = map[i];
                if channels[c_idx].quantize[mapped_index].error < min_error {
                    min_error = channels[c_idx].quantize[mapped_index].error;
                    min_index = Some((c_idx, mapped_index));
                }
            }
        }

        if let Some((c_idx, mapped_index)) = min_index {
            channels[c_idx].quantize[mapped_index].quantized_sample =
                channels[c_idx].quantize[mapped_index].quantized_sample_parity_change;
        }
    }
}
