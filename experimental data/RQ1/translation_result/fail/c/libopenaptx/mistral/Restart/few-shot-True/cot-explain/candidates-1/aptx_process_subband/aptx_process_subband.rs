...
fn aptx_prediction_filtering(prediction: &mut Box<AptxPrediction>, reconstructed_difference: i32, order: i32) {
    ...
    let predictor = clip_intp2(((prediction.s_weight[0] as i64 * prediction.previous_reconstructed_sample as i64) + (prediction.s_weight[1] as i64 * reconstructed_sample as i64)) >> 22, 23);
    ...
    for i in 0..order {
        let srd = ((*reconstructed_differences.offset(-(i+1) as isize) >> 31) | 1);
        prediction.d_weight[i as usize] -= rshift32(prediction.d_weight[i as usize] - srd * srd0, 8);
        predicted_difference += (*reconstructed_differences.offset(-i as isize) as i64) * (prediction.d_weight[i as usize] as i64);
    }
...
fn aptx_invert_quantization(invert_quantize: &mut Box<AptxInvertQuantize>, quantized_sample: i32, dither: i32, tables: &Box<AptxTables>) {
    ...
    let qr = rshift64_clip24(((qr as i64) * (1 << 32)) + ((dither as i64) * (tables.invert_quantize_dither_factors[idx] as i64)), 32);
...
fn aptx_process_subband(invert_quantize: &mut Box<AptxInvertQuantize>, prediction: &mut Box<AptxPrediction>, quantized_sample: i32, dither: i32, tables: &Box<AptxTables>) {
    let sign = diffsign(invert_quantize.reconstructed_difference, -prediction.predicted_difference);
    let same_sign0 = sign * prediction.prev_sign[0];
    let same_sign1 = sign * prediction.prev_sign[1];
    prediction.prev_sign[0] = prediction.prev_sign[1];
    prediction.prev_sign[1] = sign | 1;

    let range = 0x100000;
    let mut sw1 = rshift32(-same_sign1 * prediction.s_weight[1], 1);
    sw1 = (clip(sw1, -range, range) & !0xF) * 16;

    let range = 0x300000;
    let weight0 = 254 * prediction.s_weight[0] + 0x800000 * same_sign0 + sw1;
    prediction.s_weight[0] = clip(rshift32(weight0, 8), -range, range);

    let range = 0x3C0000 - prediction.s_weight[0];
    let weight1 = 255 * prediction.s_weight[1] + 0xC00000 * same_sign1;
