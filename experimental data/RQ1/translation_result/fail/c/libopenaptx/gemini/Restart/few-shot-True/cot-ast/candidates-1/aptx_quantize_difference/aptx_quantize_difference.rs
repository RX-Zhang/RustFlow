...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if ((a as u32).wrapping_add((1 << p) as u32)) & !((2 << p) - 1) as u32 != 0 {
        (a >> 31) ^ ((1 << p) - 1)
    } else {
        a
    }
...
fn aptx_quantize_difference(quantize: &mut AptxQuantize, sample_difference: i32, dither: i32, quantization_factor: i32, tables: &AptxTables) {
    ...
    d = rshift32_clip24(((dither as i64).wrapping_mul(dither as i64) >> 32) as i32, 7) - (1 << 23);
    ...
    let dithered_sample = rshift64_clip24(((dither as i64).wrapping_mul(interval as i64) + ((clip_intp2(mean + d, 23) as i64) << 32)), 32);
