...
fn rshift64(value: i64, shift: u32) -> i64 {
    ...
    ((value + rounding) >> shift as isize) - ((value & mask) == rounding as i64) as i64
}
...
fn rshift32(value: i32, shift: u32) -> i32 {
    let rounding = 1 << (shift - 1);
    let mask = (1 << (shift + 1)) - 1;
    ((value + rounding) >> shift as isize) - ((value & mask) == rounding) as i32
}
...
fn aptx_invert_quantization(invert_quantize: &mut AptxInvertQuantize,
    ...
    let qr = rshift64_clip24(((qr as i64) * (1 << 32) as i64) + (dither as i64) * (tables.invert_quantize_dither_factors[idx] as i64), 32);
