...
fn aptx_insert_sync(channels: &mut [AptxChannel; 2], sync_idx: &mut u8) {
 ...
 if aptx_check_parity(channels, sync_idx) != 0 {
  ...
  let mut min_channel_idx = 0;
  let mut min_subband_idx = 0;

  for c_idx in 0..2 {
   for i in 0..NB_SUBBANDS {
    if channels[c_idx].quantize[map[i]].error < min_error {
     min_error = channels[c_idx].quantize[map[i]].error;
     min_channel_idx = c_idx;
     min_subband_idx = map[i];
    }
   }
  }

  channels[min_channel_idx].quantize[min_subband_idx].quantized_sample = channels[min_channel_idx].quantize[min_subband_idx].quantized_sample_parity_change;
 }
}
