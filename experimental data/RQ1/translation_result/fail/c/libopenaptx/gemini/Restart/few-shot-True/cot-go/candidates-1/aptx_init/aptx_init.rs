...
fn aptx_reset(ctx: &mut Box<AptxContext>) {
    let hd = ctx.hd;
    unsafe {
        mem::zeroed::<AptxContext>();
    }
    *ctx = Box::new(AptxContext {
        ...
        channels: [AptxChannel {
            ...
            qmf: AptxQMFAnalysis {
                outer_filter_signal: [AptxFilterSignal { buffer: [0; 32], pos: 0 }; NB_FILTERS],
                inner_filter_signal: [[AptxFilterSignal { buffer: [0; 32], pos: 0 }; NB_FILTERS]; NB_FILTERS],
            },
        ...
        hd,
        sync_idx: 0,
        encode_remaining: (LATENCY_SAMPLES + 3) / 4,
        decode_skip_leading: (LATENCY_SAMPLES + 3) / 4,
