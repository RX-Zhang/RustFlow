...
fn aptx_reconstructed_differences_update(
    ...
    rd[p] = rd[((p as i32) + order as i32) as usize % rd.len()];
...
fn aptx_prediction_filtering(
    ...
    for i in 0..order {
        let srd = (reconstructed_differences[order - 1 - i] >> 31) | 1;
        prediction.d_weight[i] = prediction.d_weight[i].wrapping_sub(rshift32(prediction.d_weight[i].wrapping_sub(srd * srd0), 8));
        predicted_difference = predicted_difference.wrapping_add((reconstructed_differences[order - 1 - i] as i64).wrapping_mul(prediction.d_weight[i] as i64));
    }
