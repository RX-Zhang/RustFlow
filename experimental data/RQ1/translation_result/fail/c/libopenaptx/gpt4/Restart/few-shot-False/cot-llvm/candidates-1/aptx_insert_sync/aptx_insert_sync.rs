...
fn aptx_insert_sync(channels: &mut [AptxChannel; NB_CHANNELS], sync_idx: &mut u8) {
    ...
    if aptx_check_parity(channels, sync_idx) != 0 {
        for (ch_idx, c) in channels.iter_mut().enumerate().rev() {
            for &i in &map {
        ...
        let (ch_idx, subband_idx) = min_idx;
        channels[ch_idx].quantize[subband_idx].quantized_sample =
            channels[ch_idx].quantize[subband_idx].quantized_sample_parity_change;
    }
}
