...
fn aptx_invert_quantization(
    ...
    let neg_mask = if quantized_sample < 0 { -1 } else { 0 };
    let mut idx = (quantized_sample ^ neg_mask).wrapping_add(1);
    ...
    qr = rshift64_clip24(
        ((qr as i64) << 32).wrapping_add(dither as i64 * tables.invert_quantize_dither_factors[idx as usize] as i64),
        32,
    );
    ...
    factor_select = rshift32(
        factor_select + ((tables.quantize_factor_select_offset[idx as usize] as i32) << 15),
        15,
    );
