...
fn aptx_invert_quantization(
    ...
    let mut idx = (quantized_sample ^ (-(quantized_sample < 0) as i32)).wrapping_add(1);
    ...
    qr = rshift64_clip24(
        ((qr as i64).wrapping_mul(1 << 32) + (dither as i64).wrapping_mul(tables.invert_quantize_dither_factors[idx as usize] as i64)),
        32,
    );
