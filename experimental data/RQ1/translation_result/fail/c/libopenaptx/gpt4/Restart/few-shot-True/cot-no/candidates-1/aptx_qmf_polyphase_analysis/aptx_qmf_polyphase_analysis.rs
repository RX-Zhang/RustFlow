...
fn aptx_qmf_filter_signal_push(signal: &mut AptxFilterSignal, sample: i32) {
    ...
    signal.pos = ((signal.pos + 1) & (FILTER_TAPS as u8 - 1)) as u8;
}
                               ...
                               high_subband_output: &mut Box<i32>)
{
    ...
    *low_subband_output = Box::new(clip_intp2(subbands[0].wrapping_add(subbands[1]), 23));
    *high_subband_output = Box::new(clip_intp2(subbands[0].wrapping_sub(subbands[1]), 23));
}
