...
fn rshift64(value: i64, shift: u32) -> i64 {
    ...
    let mask = (1i64 << (shift.wrapping_add(1))) - 1;
...
fn rshift32(value: i32, shift: u32) -> i32 {
    let rounding = 1i32 << (shift.wrapping_sub(1));
    let mask = (1i32 << (shift.wrapping_add(1))) - 1;
...
fn aptx_quantize_difference(
    ...
    let mut interval;
    let mut inv;
    let mut sample_difference_abs = sample_difference.wrapping_abs();
    let mut error;
    ...
    interval = (intervals[1] - intervals[0]).wrapping_mul(if sample_difference < 0 { -1 } else { 1 });

    dithered_sample = rshift64_clip24(
        (dither as i64).wrapping_mul(interval as i64) + ((clip_intp2(mean.wrapping_add(d), 23) as i64) << 32),
        32,
    );
    error = ((sample_difference_abs as i64) << 20).wrapping_sub((dithered_sample as i64).wrapping_mul(quantization_factor as i64));
