...
fn clip_intp2(a: i32, p: u32) -> i32 {
    if ((a as u32).wrapping_add(1 << p)) & !(((2 as u32) << p).wrapping_sub(1)) != 0 {
        (a >> 31) ^ ((1 << p) - 1)
    } else {
        a
    }
...
fn rshift64(value: i64, shift: u32) -> i64 {
    let rounding = 1_i64 << (shift.wrapping_sub(1));
    let mask = (1_i64 << (shift.wrapping_add(1))) - 1;
...
fn rshift32(value: i32, shift: u32) -> i32 {
    let rounding = 1_i32 << (shift.wrapping_sub(1));
    let mask = (1_i32 << (shift.wrapping_add(1))) - 1;
...
fn aptx_invert_quantization(
    ...
    qr = rshift64_clip24(
        (qr as i64).wrapping_mul(1_i64 << 32) + (dither as i64).wrapping_mul(tables.invert_quantize_dither_factors[idx as usize] as i64),
        32,
    );

    invert_quantize.reconstructed_difference = (((invert_quantize.quantization_factor as i64)
        .wrapping_mul(qr as i64))
        >> 19) as i32;

    let mut factor_select = 32620_i32.wrapping_mul(invert_quantize.factor_select);
