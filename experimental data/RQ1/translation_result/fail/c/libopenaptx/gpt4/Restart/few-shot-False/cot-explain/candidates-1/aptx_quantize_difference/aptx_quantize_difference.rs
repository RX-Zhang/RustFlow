...
fn aptx_bin_search(value: i32, factor: i32, intervals: &[i32], nb_intervals: i32) -> i32 {
    ...
    while i > 0 {
        if (factor as i64).wrapping_mul(intervals[idx.wrapping_add(i) as usize] as i64)
            <= ((value as i64) << 24)
        {
...
fn aptx_quantize_difference(
    ...
    sample_difference_abs = cmp::min(sample_difference_abs, (1 << 23) - 1);
    ...
    let mut d = rshift32_clip24(((dither as i64).wrapping_mul(dither as i64) >> 32) as i32, 7)
        .wrapping_sub(1 << 23);
    d = rshift64(
    ...
    let dithered_sample = rshift64_clip24(
        (dither as i64).wrapping_mul(interval as i64)
            + ((clip_intp2(mean.wrapping_add(d), 23) as i64) << 32),
        32,
    );

    let mut error = ((sample_difference_abs as i64) << 20)
