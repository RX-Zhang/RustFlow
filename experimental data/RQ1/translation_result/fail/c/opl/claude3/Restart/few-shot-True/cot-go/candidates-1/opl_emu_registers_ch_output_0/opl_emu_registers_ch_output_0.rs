...
fn opl_emu_registers_ch_output_0(regs: &OplEmuRegisters, choffs: u64) -> u32 {
    let offset = 0xc0u64.wrapping_add(choffs) as usize;
    if opl_emu_registers_newflag(regs) != 0 {
        opl_emu_registers_byte(regs, offset, 4, 1, 0)
    } else {
        1
    }
}
