...
fn opl_emu_bitfield(value: u32, start: u32, length: u32) -> u32 {
    (value.wrapping_shr(start % 32)) & ((1u32.wrapping_shl(length % 32)) - 1)
}
...
fn opl_emu_registers_op_waveform(regs: &OplEmuRegisters, opoffs: usize) -> u32 {
    opl_emu_registers_byte(regs, 0xe0, 0, if opl_emu_registers_newflag(regs) != 0 { 3 } else { 2 }, opoffs)
}
